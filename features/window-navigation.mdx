...
Preview rendering is controlled by two settings:

| Setting | Default | Description |
|---|---|---|
| `ShowLivePreviews` | `true` | Enables or disables live DWM thumbnails. When `false`, only the app icon is shown. |
| `ThumbnailWidth` | `320` | Width of the thumbnail area in pixels. |
| `ThumbnailHeight` | `200` | Height of the thumbnail area in pixels. |

Both dimensions can be adjusted in **Settings → General**. The DWM compositor handles all scaling and clipping, so the thumbnail maintains a correct aspect ratio within the configured bounds.
...
Desktop badges are only shown in the UI when the system has more than one virtual desktop active (`HasMultipleDesktops`). If you use only a single desktop, no badge is rendered.
...
<Note>
When a desktop has no custom name set in the registry, BetterWinTab falls back to a locale-aware label that matches what Windows Task View displays — for example **Desktop 1** in English, **Escritorio 2** in Spanish, **Bureau 3** in French, **Schreibtisch 4** in German, and so on. The fallback word is resolved from `CultureInfo.CurrentUICulture.TwoLetterISOLanguageName` at runtime, so it always matches the OS display language.
</Note>
...
The **Recycle Bin** is a special built-in folder (`FolderType.RecycleBin`) that is always pinned to the bottom of the sidebar and cannot be deleted. Selecting it and pressing `Enter` opens the Windows shell Recycle Bin directly — no need to minimize windows or navigate to the desktop. It is the fastest way to access the Recycle Bin from the keyboard.
