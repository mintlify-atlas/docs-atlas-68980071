---
title: "UpdateService — Auto-Update and Download API Reference"
sidebarTitle: "UpdateService"
description: "UpdateService queries the BetterWinTab GitHub Releases API for newer versions and streams the installer to %TEMP% before launching it automatically."
---
...
The download uses a **separate `HttpClient`** with an infinite timeout (`Timeout.InfiniteTimeSpan`) and an 81,920-byte (80 KiB) streaming buffer. The filename is derived from the URL path — if the path segment is empty, it defaults to `BetterWinTab-Setup-{LatestVersion}-x64.exe`.
